...
class Detector():
    ...
    vehicle_model = DetectMultiBackend(vehicle_weights, device=device, dnn=dnn, data=data)
    stride, vehicle_names = vehicle_model.stride, vehicle_model.names
    
    helmet_model = DetectMultiBackend(helmet_weights, device=device, dnn=dnn, data=data)
    ...
    def image_preprocessing(self, image, actual_image, xmin=0, ymin=0):
        ...
        self.xmin = xmin
        self.ymin = ymin
        img_size = check_img_size(self.imgsz, s=self.stride)  # check image size
        stride = 32
        auto = True
    ...
    def vehicle(self):
        ...
        annotator = Annotator(self.actual_image, line_width=self.line_thickness, example=str(self.vehicle_names))
        
        for i,det in enumerate(pred):
            
            det[:, :4] = scale_coords(self.im.shape[2:], det[:, :4], self.image.shape).round()
            for i,det in enumerate(pred):
                det[:, :4] = scale_coords(self.im.shape[2:], det[:, :4], self.image.shape).round()
                for *xyxy, conf, cls in reversed(det):
                    vehicles[self.vehicle_names[int(cls)]].append((xyxy, self.vehicle_names[int(cls)]))
                
            for *xyxy, conf, cls in reversed(det):
                xmin,ymin,xmax,ymax = xyxy
                xyxy = (xmin+self.xmin, ymin+self.ymin, xmax+self.xmin, ymax+self.ymin)
                c = int(cls)  # integer class
                label = None if False else (self.vehicle_names[c] if False else f'{self.vehicle_names[c]} {conf:.2f}')
                annotator.box_label(xyxy, label, color=colors(c, True))
                if False:
                    save_one_box(xyxy, imc, file=save_dir / 'crops' / self.vehicle_names[c] / f'{p.stem}.jpg', BGR=True)
    ...
    def helmet(self, bbox):
        ...
        self.actual_image = np.ascontiguousarray(self.actual_image)
        annotator = Annotator(self.actual_image, line_width=self.line_thickness, example=str(self.helmet_names))
